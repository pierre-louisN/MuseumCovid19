...
$(document).ready(function()
{
	...
	//socket.emit('getNumberFiles');
	
	
	
	/*socket.on('listLabels', function(reply){
		console.log(reply);
	});*/
	...
	if(period==null) { period = "before"}; // if the period parameter is not set in URL then the default value is 'before' 
	...
	document.getElementById("infos").addEventListener("click", function (event) {
		...
		socket.emit('updateFile', nameLabel, period);
		//socket.emit('getNumberFiles', nameLabel);
		displayTreemap(nameLabel, period);
	}, false);
	
	
	function displayTreemap(nameLabel, period){
		console.log('in display treemap');
		var title = "Map of object : "+nameLabel+" "+period+" quarantine";
		//console.log(title);
		...
		if (window.location.hash === "") {
			d3.json("countries2.json", function(err, res) {
				if (!err) {
					console.log(res);
